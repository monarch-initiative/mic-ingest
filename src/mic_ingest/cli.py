"""Command line interface for mic-ingest."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """mic-ingest CLI."""
    if version:
        from mic_ingest import __version__
        typer.echo(f"mic-ingest version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for mic-ingest."""
    typer.echo("Downloading data for mic-ingest...")
...
def transform(
    ...
    """Run the Koza transform for mic-ingest."""
    typer.echo("Transforming data for mic-ingest...")
